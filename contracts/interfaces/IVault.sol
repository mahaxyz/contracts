...
interface IZaiVault {
    ...
    function boostDelegation(
        address
    ) external view returns (bool isEnabled, uint16 feePct, address callback);
    ...
    function idToReceiver(
        uint256
    ) external view returns (address account, bool isActive);
