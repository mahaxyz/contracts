...
async function main(hre: HardhatRuntimeEnvironment) {
  ...
  const lpD = await deployments.get("LP-Token-ZAIsZAI");
  const safe = await deployments.get("GnosisSafe");
  const timelockD = await deployments.get("MAHATimelockController");

  const name = "Staked ZAI/sZAI Pool"; // string memory _name,
  const symbol = "sZAIsZAI"; // string memory _symbol,
...
main.tags = ["StakingLPRewards-Curve-ZAI-sZAI"];
export default main;
